...
### Step 1: Creating iceberg table using this parquet file
...
```sql
CREATE DATABASE demo ENGINE = DataLakeCatalog('http://rest:8181/v1', 'minio', 'ClickHouse_Minio_P@ssw0rd') 
SETTINGS catalog_type='rest',warehouse='demo',storage_endpoint='http://minio:9000/warehouse-rest';
```

Create a icberg_trip tables which we will use to write:
```sql
CREATE TABLE iceberg_trips ENGINE = IcebergS3('http://minio:9000/warehouse-rest/iceberg/trips/', 'minio', 'ClickHouse_Minio_P@ssw0rd')
```

We will insert into this table from existing table trips. Let's make a note of count of no of rows in trips table.
```sql
...
```sql
CREATE TABLE default.iceberg_trips
(
    trip_id           UInt32,
    ...
    -- Iceberg has no Enum type; store as String (or map to small int) in the actual Iceberg table.
    payment_type      String,
    pickup_ntaname    String,        -- LowCardinality(String) reads fine as String
    dropoff_ntaname   String
)
ENGINE = IcebergS3(
    'http://minio:9000/warehouse-rest/ch_created',     -- table root (Iceberg directory)
    'minio',                                               -- access key
    'ClickHouse_Minio_P@ssw0rd'                            -- secret key
)
```
